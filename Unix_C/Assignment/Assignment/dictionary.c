...
int listToArrayDynamic(int rowb, Directory* DictionaryList, Dict* current, char** dictArray)
{
	/*
	...
	for(i = 0; i < rowb; i++)
	{
		dictArray[i] = (char*)malloc(sizeof(char)*(LONGEST_WORD_LENGTH));
		strcpy(dictArray[i], current->string);
	...
	if(DEBUG ==1)
	{
		printf("\n\tArray Entry 3 is: %s", dictArray[2]);
	}
	return i;
}
					...
					Directory* DictionaryList, Dict* current)
{
	...
	else
	{
		...
		*row = DictionaryList->listCount;
		if(DEBUG ==1)
		{
			printf("\n\tDictionary length is %d and %d\n", DictionaryList->listCount, *row);
		}
...
int arrayCaller(  int row, Directory* DictionaryList, Dict* current, char** dictArray)
{
	int rowb;
	rowb = row;
	row = listToArrayDynamic(rowb, DictionaryList, current, dictArray);
